...
func apply_forces(delta: float) -> void:
	if has_target:
		...
		# Rotation
			var angle_to_target : float = to_target.angle_to(Vector2.UP.rotated(rotation))
		...
		# Calculate the direction to the player
			var to_target : Vector2 = (player.global_position - global_position).normalized()
...
func _on_sense_perception_area_entered(area):
